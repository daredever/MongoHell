...
- она не стабильна
- у нас был опыт использования, но он оказался неудачным в 2013 году
- нет транзакций
- течёт память
...
Описать суть решения, показать зачем нужна монга

Рассказать этапы импорта (get from source, check if exists in mongo, insert or update to destination, update mongo).
...
- Перфоманс из коробки - вроде быстро работает.
...
- collection != table - добавить примеры.
...
- Сериализация. BSON документ (произвольные иерархические структуры данных) и простота сериализации. JObject отсутствует из коробки. BsonIgnore. Конвенции.
...
- Проблема версий пакета Mongo.Bson (DateTimeOffset) и необходимость Unit тестов на сериализацию/десериализацию.
