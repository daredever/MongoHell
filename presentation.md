...
Добрый день, друзья, рады приветствовать вас на 58 встрече Петербургского .net сообщества. 
...
- сколько из вас слышали о noSQL решениях? 
- а о MongoDB? 
...
Итак, MongoDB это кросс-платформенная документо-ориентированная база данных с открытым исходным кодом, 
относящаяся к классу NoSql (not only sql) решений.

Первый релиз был в 2009 году, а написана на с++.

#### слайд --> (картинка)
Что же мы имеем внутри mongodb? 

Для хранения данных используются JSON-подобные документы.
Важно понимать, что документ это более сложная структура, чем просто строка в реляционной базе данных.

Что в итоге? В монго дб может быть n баз данных, в каждой из которых по m коллекций, в каждой из которых может хранится l абсолютно любых документов.
...
Итак,  задача - импорт из черного ящика. Можно разбить на этапы: 
- get from source
- check if exists in db
- insert or update to destination
- update db.

Решение на PG - универсальное через хранение json в базе или жесткое под каждую систему с заранее определенной схемой. 
Как сказал Дима, тут то тнам и поможет монго дб, у которой нет жесткой схемы.

Предлагаю рассмотреть решение на примере. Показать боевой код мы к сожалению не можем, так что давайте представим, что в будущем у нас будет некая 
CRM для, например, ада и сейчас нам достаточно просто записать данные в монго дб, а потом когда-нибудь мы их прогрузим в саму CRM.
...
- Показываем в райдере пустую монгу.

- Пример репозитория CRUD. 
Показываем using для nuget пакет MongoDB.Driver
...
так как MongoDB сама выполнит всю работу.
...
- Показать результаты профилирования в консоль. примерно 10-30 мс на операцию.
- Показываем что схемы не было и автоматически создались база и коллекции через райдер. Показываем данные, обращаем внимание на _id и про ObjectId.
...
- Централизация использования технологий. Решились сменить БД перед релизом во избежание сложных миграций.
...
Рассмотрим на примере. Давайте представим у нас есть клиенты (физлица и юрлица), у каждого есть номер телефона. Нам надо разослать им смски.
...
Важное отличие монго дб от sql -  свой DSL. 
...
И еще, монго клиент поддерживает IQuerable.

#### слайд --> (Сериализация)

1. BSON документ (произвольные иерархические структуры данных) и простота сериализации.

Сериализаторы. Экстеншены: ToBsonDocument() и ToJson().  

Можно писать и регистрировать свои сериализаторы.
...
3. Конвенции - https://github.com/mongodb/mongo-csharp-driver/tree/master/src/MongoDB.Bson/Serialization/Conventions.
...
не нужны миграции + хорошая документированность
